        ...
        if (result && usersCache[interaction.member.id].quest == questNumber) {
            ...
            if (config.whitelist.questions[questNumber + 1]) {
                ...
                await guild.channels.fetch(config.whitelist.channel).then(async channel => {
                    ...
                    await channel.send({
                        content: `Allowlist de <@${interaction.member.id}>`,
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton() && (buttonValue.startsWith('approve-') || buttonValue.startsWith('fail-'))) {
        ...
        if (buttonValue.startsWith('approve-')) {
            channel.send({
                content: `✅ <@${member.id}> sua **ALLOWLIST** foi **APROVADA** em nosso servidor, \`\`PARABÉNS\`\`!, Agora aguarde para **PARTICIPAR**.`
            })
    ...
    return setTimeout(async () => {
        ...
        guild.channels.fetch(interaction.channel.id).then(channel => {
            interaction.member.send({
                embeds: [
                    new EmbedBuilder()
                    .setDescription(`🔒 **<@${interaction.member.id}>**, fechei sua sala de **ALLOWLIST** por ter passado o tempo limite para **COMEÇAR** a mesma, caso queira reabrir e tentar novamente por favor vá na sala <#1245353990431051806>`)
                    .setColor('#2f3136')
                ]
            })
