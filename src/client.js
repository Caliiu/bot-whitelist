            ...
            case "authButton":
                if (interaction.member.roles.cache.has(config.automatico.authrole)) {
                    interaction.reply({
                        embeds: [
                            new EmbedBuilder()
                                .setDescription('❌ | Você já tem acesso ao servidor')
                                .setColor("Red")
                        ],
                        ephemeral: true
                    })
                ...
                interaction.member.roles.remove(config.automatico.guestrole) // Remove o cargo de visitante
                interaction.member.roles.add(config.automatico.authrole) // Adiciona o cargo de verificado
                
                interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription(`✅ **${interaction.user.username}**, obrigado por verficiar sua conta! Seja bem-vindo(a)!`)
                            .setColor("Green")
                    ],
                    ephemeral: true
                })
...
client.on('ready', async () => {
    ...
    setInterval(() => {
        ...
        channel.send({
            embeds: [
                new EmbedBuilder()
                    .setDescription('🔔 Leia nossas <#1242994901625602143> para garantir o envio de uma boa **ALLOWLIST.**')
                    .setColor("#2f3136")
            ]
        })
    }, 60 * 60000)
...
client.on("interactionCreate", (interaction) => {
    if (interaction.isButton()) {
        if (interaction.customId === 'criarcall') {
            ...
            let canalquevaideletar = client.channels.cache.find(c => c.name == `suporte-${interaction.user.username}`)
            ...
            if (interaction.guild.channels.cache.find(call => call.name === `suporte-${interaction.user.username}`)) {
                let call = interaction.guild.channels.cache.find(call => call.name === `suporte-${interaction.user.username}`);
                interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription(`> *Você já possui uma call criada!* ${call}.\n *clique em* **DELETAR** *ela e crie uma nova com a configuração desejada!*`)
                            .setColor(0x2f3136)
                    ],
                    ephemeral: true
                })
