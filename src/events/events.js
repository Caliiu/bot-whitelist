...
client.on('guildMemberRemove', async (member) => {
    ...
    if (exit_channel) {
        ...
        const [
            [ticket]
        ] = await mysql.query(`SELECT * FROM ${bot_table} WHERE discord_id = ? AND is_finished = 0 AND type = 'ticket'`, [member.id])
        if (ticket) {
            ...
            if (id && id > 0) {
                ...
                message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(0x00FF00)
                        .setDescription(`**<@${message.author.id}> Passaporte Liberado, Seja Bem-Vindo(a) a NacionalPlayerGG**`)
                    ]
                ...
                return message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                        .setDescription(`<@${message.author.id}> Envie apenas o **PASSAPORTE** para liberação da sua whitelist`)
                        .setColor(0xFF0000)
                    ]
                }).then(msg => {
                    setTimeout(() => msg.delete(), 3000)
...
client.on('channelDelete', async (channel) => {
    const [
        [ticket]
    ] = await mysql.query(`SELECT id FROM ${bot_table} WHERE channel_id = ? AND is_finished = 0 AND type = 'ticket'`, [channel.id])
    if (ticket) {
    ...
    const [
        [whitelist]
    ] = await mysql.query(`SELECT id FROM ${bot_table} WHERE channel_id = ? AND is_finished = 0 AND type = 'whitelist'`, [channel.id])
    if (whitelist) {
        await mysql.query(`DELETE FROM ${bot_table} WHERE channel_id = ? AND is_finished = 0`, [channel.id])
    }
})